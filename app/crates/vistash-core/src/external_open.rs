//! 默认程序打开的只读临时副本管理器（任务 5.6，调研 §4–§5 冻结的隔离模型）。
//!
...
impl ExternalOpenManager {
    ...
    pub fn prepare(&self, lib: &Library, hash: &ContentHash) -> Result<PathBuf> {
        ...
        let tmp = self.session_dir().join(format!("{}.part", target.file_name().expect("由显示名拼出，必有文件名").to_string_lossy()));
    ...
    fn read_manifest(&self) -> SessionManifest {
    ...
    fn write_manifest(&self, manifest: &SessionManifest) -> Result<()> {
        ...
        let text = serde_json::to_string_pretty(manifest)
            .map_err(|e| AppError::detailed(Code::LibraryIoFailed, format!("序列化清单失败：{e}")))?;
        std::fs::write(self.manifest_path(), text).map_err(|e| {
            AppError::detailed(Code::LibraryIoFailed, format!("写会话清单失败：{e}"))
        })
    }

    /// 启动时清理过期会话。边界见模块文档；报告列出删了什么、什么被占用留待重试。
    pub fn cleanup(&self, now: DateTime<Utc>) -> CleanupReport {
        ...
        let expiry =
            chrono::Duration::from_std(SESSION_EXPIRY).unwrap_or_else(|_| chrono::Duration::hours(24));
        let cutoff = now - expiry;
        let expired: Vec<String> = manifest
            .iter()
            .filter(|(session_id, created_at)| {
                // 当前会话无论多旧都不得自删：本运行还可能在引用其中的副本。
                session_id.as_str() != self.session_id
        ...
        for session_id in expired {
            ...
            match remove_session_tree(&dir) {
                ...
                Err(()) => report.deferred_sessions.push(session_id),
            }
        }
        // 清单收缩尽力而为：写失败不影响本次报告的真实性。
...
/// 下次重试会先清一遍属性再继续。
fn remove_session_tree(dir: &Path) -> std::result::Result<(), ()> {
...
fn clear_readonly_recursive(dir: &Path) -> std::result::Result<(), ()> {
...
mod tests {
    ...
    fn generated_session_ids_are_canonical_uuid_v7() {
        ...
        let parsed =
            crate::ids::parse_canonical_uuid(&id, crate::error::Code::PromptIdInvalid)
