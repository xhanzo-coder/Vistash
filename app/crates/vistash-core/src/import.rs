...
//! 查重必须以侧车为准而不是以本体为准。
//!
//! 本次全部串行执行。并行化只会替换执行器，不改变本文件的任何契约。
...
/// 完全一致，否则两条路径会慢慢分叉。
/// 物化阶段的载荷来源：磁盘文件或内存 PNG 字节（剪贴板位图，约束）。
///
...
/// 缩略图，且这件事不会有任何报错——这正是"调整该值的代价是一次全库重建"
/// 那句话得以成立的机制。
...
pub enum ImportSource {
    ...
    Directory(PathBuf),
    /// 剪贴板位图经 Rust 侧编码的 PNG 字节。没有文件系统来源：
...
/// 调用方提前宣布同样算冒充。`stopping` 表示停止命令已被
/// 接受但协调器还没在安全边界退出。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
