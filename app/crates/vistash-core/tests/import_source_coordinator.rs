//! 统一 [`ImportSource`] 协调器的目标、层级与停止合同。
//!
//! 约束：文件选择、目录选择、拖放与剪贴板只负责产生 `ImportSource`，之后进入
//! 同一 Rust 导入协调器。各组测试覆盖下面的行为要求：
//!
//! 1. **文件导入目标**：文件导入到
...
//! 5. **任务身份与库级并发**：每个长任务拥有唯一 `TaskId` 与库级
//!    并发键；同一时刻一个库只允许一个进行中的导入；
//! 6. **扫描阶段停止**：目录扫描尚未开始写入时
//!    必须尽快停止，库内不留任何痕迹；
