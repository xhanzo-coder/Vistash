//! 生产链路切换到 v3 后的行为合同。
//!
//! 测试使用迁移后的 v3 fixture 验证索引重建、双文件名查询与 Catalog 单归属事务，
//! 同时覆盖生产别名、SQLite、Catalog 和批量命令的组合行为。
//! 三组契约各自对应一条规格要求：
//!
...
///
/// 迁移完成后以生产门禁 [`Library::open`] 重新打开——这同时验证生产链路的
