            ...
            for theme in ["dark", "light"]:
                for width, height in [(1440, 900), (760, 600)]:
                    ...
                    try:
                        ...
                        scope.get_by_role("button", name="定位色卡", exact=True).click()
                        ...
                        expect(scope.get_by_text("已保存", exact=True)).to_be_visible()
                        scope.get_by_role("button", name="定位文件信息", exact=True).click()
                        expect(scope.get_by_role("button", name="文件信息", exact=True)).to_be_focused()
