...
- [x] 10.4 实现提示词主要字段显式保存/`Ctrl+S`、取消、未保存导航拦截、保存失败草稿保留与备注独立自动保存
  - 2026-08-22 落地：`PromptBodyFocus` 从只读聚焦阅读升级为显式保存编辑器——正文/标题/模型/参数只在"编辑主字段"进入的明确编辑状态中修改，保存按钮与 Ctrl+S 共用同一条 update_prompt 写入路径（可空字段空串归一为 null），成功后 onSaved 触发权威刷新并保持编辑态显示"已保存"；失败不退出编辑、不丢草稿、就地呈现稳定错误码；取消=放弃修改回只读。未保存导航拦截分两层：折叠编辑器（返回列表/Esc）在脏状态弹出保存并离开/放弃修改/留在当前页三选一对话框（焦点落在留在当前页）；跨层导航经新建 `draftGuard.ts` 单槽位脏探针——App 在一级入口切换与 Tauri onCloseRequested（特性检测 __TAURI_INTERNALS__，非 Tauri 环境跳过）放行前查询，脏则拦下并请求编辑器拉起同一对话框。"切换素材"触发在结构上不可达：编辑发生在占满中央区的聚焦视图里，集合视图不可达（落地说明记入组件注释）。备注独立自动保存：把 AssetNoteEditor 的状态机抽成共享 `workspace/NoteAutoSaveEditor.tsx`（停止输入 800ms/失焦/Ctrl+Enter 三触发、失败保草稿、卸载补写），图片侧改为薄包装（原 5 项测试零改动通过），提示词侧 `PromptNoteEditor` 注入 set_prompt_note 与 aria-label 接入检查器 note 分区。测试新增 16 项：编辑预填/归一化载荷+权威刷新/失败保草稿/Ctrl+S/干净直接退出/三选一全路径/Esc/探针拦截注销、备注防抖写入与失败重试、工作区编辑入口到 update_prompt 回路。
- [x] 10.5 实现关联图片库内多选器、本地拖入/选择导入、解除、已删除状态、设为封面与封面回落
  - 2026-08-22 落地：后端新增 `Catalog::linked_image_states`（按 prompt_row.linked_image_hashes 权威顺序逐哈希查 `asset_is_deleted`，返回 `LinkedImageState{hash,deleted}`）与同名 Tauri command，配回收站态往返测试（删除→关联保留且 deleted=true→还原→false；PromptNotFound 如实传播）。前端新建 `PromptImageLinks` 承担检查器 images 分区全部职责（Inspector 以活动项为 key 重挂载）：库内多选器复用 `catalog_snapshot`（location=active）并过滤已在关联里的候选，确认走单次批量 link_images；"从本地导入"按钮经 `pickImageFiles`（对话框扩展名白名单与核心 MediaType 一致：png/jpg/jpeg/webp/gif/bmp）；拖入与选择汇入同一条 import_and_link 编排，逐项呈现 已导入并关联/库内已有已关联/已入库但未关联 + 稳定错误码。窗口级拖放互斥用 `promptDropZone.ts` 单槽位注册表解决——Tauri 拖放事件广播给所有监听者且触发顺序无保证，App 在默认整库导入分发前查询"最新落点是否被关联区认领"，判定只读共享坐标快照故与顺序无关（over→move 映射、devicePixelRatio 换算、注销即回到默认语义均有测试）。已删除关联图显式带"已删除"徽标保留在列（绝不静默隐藏），设为封面只对正常项提供；检查器封面解析为显式 cover_image_hash 优先、缺省取第一张正常关联图（徽标与 aria-label 均按 effectiveCover 统一），解除显式封面/永久删除后的回落由权威层保证。已知边界：卡片瀑布流的缺省封面回落暂不跳过回收站里的缺省封面（resolved_cover 维持瀑布流语义），规格钉死的回落场景只覆盖解除与永久删除两条路径，均已由第 7 章后端实现。任何关联变更经 onChanged 触发工作区权威刷新（卡片封面与计数随之更新）。测试：promptDropZone 5 项 + PromptImageLinks 7 项（多选排除已关联、import_and_link 载荷与逐项报告、错误码呈现、拖入命中才接管落点、显式/缺省封面并存与取消），检查器侧补 已删除 徽标与异步加载冲刷。
- [x] 10.6 实现提示词回收站、还原缺失文件夹警告、二次确认 purge 与图片不变呈现
  - 2026-08-22 落地：后端 delete_prompt/restore_prompt/purge_prompt_trash 与 PromptRestoreOutcome/PromptPurgeReport 在第 7 章已建成，本任务补齐呈现与交互。检查器组织分区按 trashLocation 分叉——回收站位置让位给"还原提示词"入口（正文、组织与关联原样保留的说明文案，不再提供文件夹勾选与移入回收站），正常位置在标签编辑之后提供"移入回收站"危险按钮；移入与清空都经二次确认对话框发起。工作区新增回收站工具条（trash_count=0 或忙碌时禁用"清空回收站"）、逐项清理报告（"已永久删除 N 条，失败 M 条" + 失败条目标题/ID + ErrorLine 稳定错误码）与还原缺失文件夹的非阻断警告（trash.restore_target_folder_missing + detail 列出缺失路径；还原本身成功，恢复仍存在的路径、其余落回提示词根位置）。把图片侧内联的 ConfirmDialog 提取为 workspace/ConfirmDialog 共享组件（挂载即聚焦取消、busy 双禁用，DOM 不变，图片侧测试零改动通过）；顺带修正 errorText 中该码的文案——原文"还原失败"与实际"还原成功但部分归属落回"相悖，归档设计明确要求"显式告知"。图片不变呈现以测试钉住：整个清空流程的 IPC 记录中不含任何图片写命令（delete_asset/purge_trash/set_asset_*/batch_*），图片素材只经后端既有不变量保护（关联移除、文件不动）。测试新增 5 项：清空二次确认（数量文案、取消默认聚焦、取消零写入、确认后报告+权威刷新、无图片写命令）、回收站还原入口替换与缺失文件夹警告、正常区移入回收站回路，检查器侧还原/删除两条上报路径。
...
- [x] 11.2 在公开 Workspace/Inspector seam 建立组件测试，覆盖双库布局恢复、四视图、单/多选、批量操作、草稿、关联、搜索、两类回收站与错误码
  - 2026-08-23 落地：本任务同时补上 8.5 留下的 specs MUST 缺口——批量工具条当时只有外壳没有接线。新建 `workspace/BatchOrganizer.tsx` 只做意图翻译（文件夹/标签/收藏/关联/移入回收站，零 IPC），挂进两侧 Inspector 的多选分区：`data-inspector-section="batch"` 按共同值呈现动作（共同标签给"批量移除标签 X"按压态、部分共有给"批量加入文件夹"勾选态、新标签表单、收藏切换），`batch-links` 分区自取跨库候选（图片侧用 prompt_snapshot 取提示词候选，提示词侧用 catalog_snapshot 取图片候选；提示词侧后端无批量命令，工作区以 Promise.all 逐条 link_images 并聚合成统一 BatchReport，逐项 try/catch/finally 推进 BatchProgress），`batch-danger` 放批量移入回收站（经二次确认）；回收站内多选刻意只给数量摘要（还原是单数语义）。两个工作区新增 runBatch/requestBatchDelete 协调写入与报告，报告按条呈现 display_name+ErrorLine（设计第六条逐项失败隔离）。样式补齐 .batch-organizer/.batch-link-picker/.compact-form 的真实 token。测试侧：两侧 Inspector 各加 4 项多选分区测试，AssetWorkspace 加批量工具条+分区上报完整选中集合、批量删除经确认两项，PromptWorkspace 加批量打标/批量关联逐项失败聚合/批量删除三项；双库布局恢复 seam 测试暴露真 bug 两枚——useScrollRestore 在 savedOffset=0 的预读取回退时就消费恢复键导致首挂载恢复丢失（改为只在真正应用偏移时记账），换库时上一库滚动位置残留在 DOM（四个集合视图以 `key={libraryId ?? "no-library"}` 重挂载，等该库自己的 read_layout 返回后再恢复）；另把工作区测试的 scrollTop 几何桩从共享单值改为 WeakMap 按元素存储，否则重挂载的新 DOM 会读到旧元素偏移、掩盖真实语义。覆盖核对：双库布局恢复（新 seam 测试）、四视图（瀑布流/详情列表既有套件）、单/多选（SelectionModel 与两工作区测试）、批量操作（batchToolbar + 新 seam）、草稿（笔记编辑器）、关联（关联分区）、搜索（GlobalSearch + 芯片）、两类回收站（两侧 trash 测试）、错误码（data-error-code 断言贯穿）。四道门禁通过（前端 lint/typecheck 干净、204 测试全绿、Rust 277 测试零失败、clippy -D warnings 干净）。
- [x] 11.3 完成 ARIA grid/listbox 键盘模式、roving focus、方向键/Home/End、焦点圈、对话框焦点陷阱/Esc/触发器归还与高对比/缩放验收
  - 2026-08-23 落地：键盘模式盘点后补齐三块缺口。①新建 `workspace/dialogFocus.ts`（useDialogFocusTrap）统一落实模态对话框三条纪律——Tab/Shift+Tab 全托管循环（每站手动计算并 preventDefault，不依赖浏览器自然 Tab 序，jsdom 与真实环境行为一致）、Esc 走安全侧并阻断冒泡（底层容器的全局 Esc 语法如清空选择/退出聚焦模式不会被连带触发）、卸载时焦点归还打开前的触发元素；接入全局 ConfirmDialog（busy 时 Esc 同样忽略）与 PromptBodyFocus 的脏草稿三选一对话框（后者抽出为独立挂载单元 DraftGuardDialog，钩子才能在挂载期捕获触发器）。②两个瀑布流容器补 `role="listbox" aria-multiselectable aria-orientation="vertical"`、卡片命中区补 `role="option"`，与详情列表既有 grid 模式并列构成任务要求的两种 ARIA 键盘模式；方向键/Home/End/Shift 范围与 roving focus 在 8.5/9.x 已落地（SelectionModel + useRovingFocus），本轮仅以测试钉住。③styles.css 增设 `prefers-contrast: more` 分支把 --border-subtle 换档到强边界（纯 token 引用，无裸颜色），WCAG 对比度门禁由既有 designTokens.test.ts 机器验证（文本 4.5:1、非文本 3:1）。缩放验收核对：字号/间距全部相对单位（rem/clamp），无固定 px 字号，200% 缩放下 flex/grid 布局自然重排。新增 ConfirmDialog.test.tsx 四条（默认聚焦取消、双向 Tab 圈、Esc 安全侧/busy 忽略、关闭归还触发器）与 PromptBodyFocus 一条（对话框内 Esc=留在当前页不冒泡、Tab 圈住三个动作）；两瀑布流测试各并入 listbox 角色断言。四道门禁通过（前端 lint/typecheck 干净、209 测试全绿、Rust 283 测试零失败、clippy -D warnings 干净）。
- [x] 11.4 记录 10,000 图片瀑布流、10,000 图片列表、10,000 长提示词卡片/列表的 DOM 峰值、首屏、快速滚动、内存与视图切换 release 基线
  - 2026-08-23 落地：按 design.md「性能」口径（生产门禁以 release WebView2 重测为准）建立可复现测量——`app/scripts/perfFixture.mjs` + `measure-perf.mjs` 用 playwright-core 驱动系统 Microsoft Edge（与 WebView2 同为 Chromium 内核）加载 `pnpm build` 的 dist，经 addInitScript 注入 `window.__TAURI_INTERNALS__` 内存桩应答 10,000 条素材/长提示词 fixture（含真实 1×1 PNG 解码与 blob 缩略图生命周期），四场景 × 六指标：首屏（导航到集合项出现）、集合 DOM、快速滚动双帧（45 次 × 900 px）、DOM 峰值、JS heap、视图切换耗时。**测量暴露并修复了一个 release 真实缺陷**：视口固定布局的高度链未封顶（html/body/#root 仅 min-height、.app-shell min-height:100vh、.app-main 与工作台容器缺 min-height:0、workspace grid 行为 auto），集合容器被内容撑到全高（约 214 万 px），overflow-y:auto 永不触发，虚拟化把 10,000 项全部渲染进 DOM；jsdom 几何桩把高度 mock 成 800px 所以单测全程不可见。修复 styles.css 高度链（html/body/#root height:100%、.app-shell height:100vh、.app-main min-height:0、两工作台 minmax(0,1fr) 栅格行、.catalog-main flex 列化、左右栏各自 overflow），非工作台的 shell 直属 section 补 overflow-y:auto 保住库选择页滚动。修复后基线（scripts/perf-baseline.json）：图片瀑布流 首屏 295ms/DOM 12/双帧均值 14.32ms 峰值 29.1ms/峰值 DOM 22/heap 47.51MiB/切换 37ms；图片详情列表 325ms/18/13.33/14/25/51.70/26；提示词卡片瀑布流 333ms/14/13.69/13.9/28/48.86/21；提示词详情列表 382ms/18/13.85/14.3/25/51.58/26——全部与原型阶段同量级且优于其数字，DOM 峰值从 10,000 回到两位数。oxlint ignorePatterns 增加 scripts 目录（测量脚本依赖无类型上下文的第三方与浏览器全局，类型感知规则在此只会产生噪声）。四道门禁通过（前端 lint/typecheck 干净、209 测试全绿、Rust 277+6 测试零失败、clippy -D warnings 干净）。
- [x] 11.5 在 E 盘临时库复制走通 v1→v2 迁移、两库组织、备注/收藏、多选、关联、封面、搜索、删除/还原/purge 的 Windows release 流程
  - 2026-08-24 落地：新增 `scripts/e2e-release.mjs`，经 `pnpm tauri build --no-bundle` 生成真正内嵌 `frontendDist` 的 production exe（直接 `cargo build --release` 不注入 Tauri production 环境，仍会加载 `devUrl`，已从运行说明删除）。脚本以 WebView2 CDP 驱动真实 Tauri/Rust IPC，并在 `E:\vistash-release-e2e\v1-library` 的 8 张正常图片 + 1 张回收站图片 fixture 上走通：迁移阻塞页与 v2 落盘、图片文件夹/备注/收藏/批量标签、Ctrl+F/Ctrl+K、提示词文件夹创建与归属、两图普通关联/显式封面、提示词备注/收藏、图片删除→还原→purge、提示词删除→还原→再删除→purge。真实验收暴露并修复共享快照竞态：删除后的 active refresh 迟到会覆盖已切换的 trash 快照；`useWorkspaceSnapshot` 现在按 request key 拒绝过期响应，并有 Workspace seam 回归测试。使用者 `%APPDATA%` 配置改为原子 rename 备份，进程确认停止后恢复；4 项 Node 文件系统测试覆盖有/无原配置、停止失败不删配置及异步启动失败仍恢复。复审修复全部落地后再次运行，最终 `e2e-report.json` 仍为“全部场景/通过”，截图与诊断现场位于 `E:\vistash-release-e2e\shots`，收尾后无 `.e2e-backup` 残留。
- [x] 11.6 运行 `pnpm lint`、`pnpm typecheck`、`pnpm test`、Clippy 与全部 Rust 测试，并修正全部问题
  - 2026-08-24 落地：修复审查与 release E2E 暴露的问题后串行通过全部正式门禁：`pnpm lint`（oxlint type-aware）零诊断，`pnpm typecheck` 零错误，`pnpm test` 为 35 个 Vitest 文件 229 项测试全绿并追加 4 项 Node 配置安全测试，`cargo clippy --workspace --all-targets -- -D warnings` 零警告，`cargo test --workspace` 为 `vistash-core` 278 项 + Tauri 6 项、doc tests 零失败。production 构建另经 `pnpm tauri build --no-bundle` 成功，真实 release E2E 全场景通过。
- [x] 11.7 使用 `web-design-guidelines` 审查信息层级、焦点、窗口适配、文本溢出、异步状态、虚拟化和语义主题结构
  - 2026-08-24 落地：按当日最新版 Vercel Web Interface Guidelines 全量复核 `App`、双工作台、四种集合视图、检查器、对话框/抽屉、关联图片区与 `styles.css`。修正：`PromptCoverImage` 与关联缩略图补显式宽高防 CLS；抽屉补 `overscroll-behavior: contain`；工作台/检查器/空状态标题补 `text-wrap: balance` 与长文本换行；计数、尺寸、详情行、进度与报告使用 tabular nums；按钮明确 tap highlight。既有门禁继续保证：skip link/主内容目标、focus-visible、对话框焦点陷阱、reduced motion、语义 token、虚拟化、图片 alt 与表单 label。另以 Python Playwright 连接真实 production WebView2：页面无水平溢出，两枚 `role=separator` 可见，键盘 ArrowRight 使栏宽 240→256，折叠分类栏后可用键盘展开，浏览器零 error；审查截图保存为 `E:\vistash-release-e2e\shots\13-layout-audit.png`。修正与复审回归后 lint/typecheck/229+4 前端测试全绿。
- [x] 11.8 执行双轴 `code-review`，分别核对仓库规范与本 change 的 specs/design，消解全部发现
  - 2026-08-24 落地：以 `main`/`52debe9` 为固定点并纳入当前工作树，按 Standards 与 Spec 两轴多轮并行复审，逐项消解全部发现。核心修正包括：备注卸载写入用 retained revision 防止旧失败/旧成功覆盖或删除新草稿；E2E `spawn` 异步错误进入配置恢复生命周期并聚合场景/恢复双错误；查询/快照与 pane presentation/frame/expand controls 分别收敛为共享深模块；同 `library_id` 的整表布局写串行、读取等待 pending value，冷启动 ready 前阻塞工作台与全局 locate 写入；全局定位在 SelectionModel 真正消费后 ack nonce 并由 App 清除；草稿保存/放弃后继续最初的切换/定位/关闭 continuation；检查器与卡片统一只把正常关联图片解析为有效封面。最终复审两轴均明确报告“无发现”。
- [x] 11.9 运行 `openspec validate design-dual-asset-workspace --strict --no-interactive` 并修正全部问题
  - 2026-08-24 落地：全部实现、release E2E、Web Interface Guidelines 审查、正式门禁与最终双轴复审完成后运行 `openspec validate design-dual-asset-workspace --strict --no-interactive`，结果为 `Change 'design-dual-asset-workspace' is valid`；`git diff --check` 与三个新增 Node 脚本语法检查同时通过。
