...
回收站报告按 QueryClient 生命周期和库身份保存在模块内部订阅状态中；切库后返回仍可查看旧请求的结果，关闭报告才移除，不把报告塞进服务端查询缓存。延续 Archive Desk、既有深模块边界和同库组织 mutation 范围。普通删除仍是移入库内回收站而非永久删除；回收站组织信息继续只读。全局任务中心接线仍属于 10.3，不提前勾选该任务。
...
应用级运行时由 `app/runtime.ts` 创建一次 `appPlatform` 与 `appTaskCenter`。图片模块保留窄公共 interface，不把平台或任务中心塞入 `AssetLibraryWorkspace` props；任务中心是外壳与图片模块共享的应用 seam，后续默认入口切换时由同一实例接入 `AppShell`。每次导入先注册 `TaskKind="import"`、库级传输并发键和可停止标志，再把后端进度映射到 TaskCenter；重复导入只返回稳定 `transfer.already_running`，不静默排队。

任务中心记录使用 TaskCenter 生成的应用任务 ID；收到首个传输进度后保存后端 `task_id`，停止按钮只能在存在该 ID 时出现。停止先进入 `stopping`，后端返回 `stopped` 且导入结果到达后才调用 `confirmStopped`；停止请求失败保留“正在停止”并在任务项附近显示错误，不冒充已停止。导入完成报告区分成功、跳过、失败和尚未处理，失败显示来源文件名与错误码。
...
备选方案是让 `App.tsx` 继续接收路径、进度和刷新版本，或让每个导入按钮各自调用 IPC；前者会把旧外壳协议带入新模块，后者会分裂重复与失败语义，均拒绝。当前导入报告先由共享任务中心呈现，详细停止/冲突对话与完整任务详情在 10.3 继续扩展，不为本阶段添加第二套任务 store。

### 27. 传输任务详情与图片出站操作

导入与导出共享应用级 `appTaskCenter` 和库级传输并发键。图片模块在同一个 transfer handle 中保存应用任务 ID、后端 task ID、停止请求与后端确认；任务中心只呈现状态，不决定事务。首个后端进度到达前不显示停止按钮，收到 ID 后才允许停止；请求失败保留 `stopping` 并在任务项附近显示稳定错误码，允许再次请求。结果到达且后端返回 `stopped` 后才进入 stopped，成功、跳过、失败、未处理和逐项错误明细都保留在任务记录中。应用壳层任务中心通过公开的图片模块停止协调器复用同一语义，关闭详情不会停止任务。
...
出站控制与空集合引导共用 Archive Desk token 和既有 Button/Dialog/Popover。复制与打开的入参形状分别是单个 `AssetId`，因此多选合成在类型和界面两端都被排除；所有长任务都能从同一个任务中心观察。
...
- 顶栏“导入”是正常库的唯一常驻导入入口，菜单明确包含图片、文件夹和剪贴板；工作区仍认领 Ctrl+V、拖放并显示稳定错误或拖放状态，但非空集合不重复显示三枚导入按钮。空集合引导继续保留三种入口，因为此时导入就是主要任务。
