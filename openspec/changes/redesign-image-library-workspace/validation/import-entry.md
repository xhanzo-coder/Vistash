...
- 入站任务注册 `TaskKind="import"` 与库级传输并发键。内部任务状态记录导入标题与传输进度；失败或未处理结果由图片工作区就地持续显示成功/失败/未处理计数和逐项错误码。
...
`app/scripts/import-entry-check.py` 使用 Python Playwright 与 Windows headless Edge，覆盖深/浅主题、1440×900/760×600。四组全部通过：文件选择、文件夹选择、剪贴板就地反馈、文本框粘贴隔离、空结果导入引导和无水平溢出。测试只使用开发内存库，未访问真实素材。
