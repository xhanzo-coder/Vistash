...
- 应用级 `app/runtime.ts` 提供一次性的 Tauri platform 与 TaskCenter；顶栏导入菜单通过 typed `AssetImportRequest` 投递到图片模块，不复制第二套导入协调器。
...
`app/scripts/app-shell-integration-check.py` 使用 Edge headless 和 100 条内存 fixture，验证新版默认 App 的图片/提示词一级切换、顶栏导入菜单、全局搜索定位、任务中心、设置 Dialog、水平溢出与页面错误。报告和截图位于 `app/artifacts/app-shell-integration/`，流程通过。
