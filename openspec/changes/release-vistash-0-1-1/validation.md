...
日期：2026-09-01。候选基于最新 `main` 的图片—提示词关系管理修复，当前只用于测试，不是公开 Release。
...
```powershell
node scripts/installer-lifecycle.mjs --nsis target/release/bundle/nsis/Vistash_0.1.1_x64-setup.exe --upgrade-nsis E:\vistash-release-e2e\upgrade-candidates\Vistash_0.1.1_x64-setup.exe
```

结果：隔离配置下安装、启动并通过 WebView2 CDP 确认进入图片工作区且打开 `library` 测试库；同版本重装后再次打开；此前已验证 0.1.0→0.1.1 补丁升级和卸载/重装后设置与素材库保留。本次当前 0.1.1 候选再次完成卸载清理，真实 `%APPDATA%\com.vistash.app` 已恢复，隔离目录已删除。
...
- 当前候选文件只在本机 `target/`，未创建 `v0.1.1` tag 或 GitHub Release。
- 正式候选必须从合并后的清洁 `main` 提交创建受 ruleset 保护的 annotated `v0.1.1`，并由现有 `release.yml` 生成草稿 Release。
